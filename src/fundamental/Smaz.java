...
public class Smaz {
    ...
    public byte[] compress(String inString) {
        String result;
    	if ((result=formalizeIntoAsciiOnly(inString))!=null){
    		// to do: append the original  and changed strings to log file
    		//      throw new IllegalArgumentForSmazException(input, output);
        	inString=result;
        }
    ...
    private String formalizeIntoAsciiOnly(String str) {
        ...
        if (!changed){
        	...
        	log.error("\""+str+"\" has been changed to \""+ newStr+"\"");
        	return newStr;        	
        }
    }
